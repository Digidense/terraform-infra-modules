...
#}
#
## Lambda Function (only created if auth_type is LAMBDA)
...
#  role             = aws_iam_role.lambda_exec_roles[count.index].arn
#  source_code_hash = filebase64sha256(var.lambda_function_code)
#}
#
## IAM Role for Lambda Execution (only created if auth_type is LAMBDA)
...
#}
#
## Attach Basic Execution Policy to Lambda Role (only created if auth_type is LAMBDA)
#resource "aws_iam_role_policy_attachment" "lambda_basic_execution" {
#  count = var.auth_type == "LAMBDA" ? 1 : 0
#
#  role       = aws_iam_role.lambda_exec_roles[count.index].name
#  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
#}
#
## Attach Lambda Execute Policy (only created if auth_type is LAMBDA)
...
#}
#
## Allow API Gateway to Invoke the Lambda Function (only created if auth_type is LAMBDA)
#resource "aws_lambda_permission" "apigw_invoke_lambda" {
#  count         = var.auth_type == "LAMBDA" ? 1 : 0
#  statement_id  = "AllowAPIGatewayInvoke"
#  action        = "lambda:InvokeFunction"
#  function_name = aws_lambda_function.auth_lambda[count.index].function_name
...
#  rest_api_id     = aws_api_gateway_rest_api.api.id
#  authorizer_uri  = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${aws_lambda_function.auth_lambda[0].arn}/invocations"
#  type            = "REQUEST"  # Use "REQUEST" for a Lambda authorizer
#}
#
...
#  authorizer_id     = var.auth_type == "LAMBDA" ? aws_api_gateway_authorizer.lambda_authorizer[0].id : null
#  request_models = {
...
#}
#
## API Gateway Integration for IAM (only created if auth_type is IAM)
...
#}
#
## API Gateway Integration for Lambda (only created if auth_type is LAMBDA)
...
#  type                    = "AWS_PROXY"
#
#  uri = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${aws_lambda_function.auth_lambda[0].arn}/invocations"
#
#  depends_on = [
